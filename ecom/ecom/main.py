...
def sell_product():
    ...
    try:
        cursor.execute('INSERT INTO products (productname, price, productcategory, productdescription, productimage, ownerid) VALUES (%s, %s, %s, %s, %s, %s)',
                       (product_name, product_price, product_category, product_description, image_path, session['id']))
        conn.commit()
...
def fetch_products():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute('SELECT productid AS id, productname AS name, price, productcategory AS category, productdescription AS description FROM products WHERE ownerid = %s', (session['id'],))
