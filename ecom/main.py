...
def get_products():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT productid, productname, price, productcategory, productdescription, productimage, productstock FROM products WHERE ownerid = %s', (session['id'],))
